...
import { ScrapeJob, ScrapedPost, ScriptExecution } from "../models/index.js";
...
export const exportData = async (req, res) => {
  ...
  try {
    ...
    if (format === "json") {
      ...
      res.setHeader("Content-Disposition", `attachment; filename=job_${targetId}_export.json`);
    ...
    if (format === "csv") {
      ...
      res.setHeader("Content-Disposition", `attachment; filename=job_${targetId}_export.csv`);
    ...
    if (format === "xlsx") {
      ...
      res.setHeader("Content-Disposition", `attachment; filename=job_${targetId}_export.xlsx`);
      return res.send(buffer);
    }

    if (format === "md") {
      let mdContent = `# Scrape Report for Group: ${job.group_url}\n`;
      mdContent += `- **Job ID**: ${targetId}\n`;
      ...
      res.setHeader("Content-Disposition", `attachment; filename=job_${targetId}_export.md`);
    ...
    if (format === "pdf") {
      ...
      res.setHeader("Content-Disposition", `attachment; filename=job_${targetId}_export.pdf`);
